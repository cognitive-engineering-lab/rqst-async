...
fn chat_gives_reply() {
    ...
    assert_eq!(
        ...
        "Success",
        r#"Reply did not contain "type": "Success""#
    );
}
...
#[test]
fn chat_can_be_canceled() {
    let server = Server::start();
    let body = thread::scope(|s| {
        let req = s.spawn(|| server.post("/chat", r#"{"messages": ["Hello, world!"]}"#));
        let (status, body) = server.post("/cancel", "");
        assert_eq!(status, 200, "POST /cancel responded {status}: {body:?}");

        let (status, body) = req.join().unwrap();
        assert_eq!(status, 200, "POST /chat responded {status}: {body:?}");
        body
    });
    ...
    let result =
        serde_json::from_str::<serde_json::Value>(&body).expect("/chat returned invalid JSON");
    assert_eq!(result, json!({"type": "Cancelled"}));
}
